...
function renderMarketBasics() {
  ...
  $('yesPrice').textContent = yes ? `${displayCents(yes)}${formatSignedChange(predContext.yesChange, '¢')}` : '—';
  $('noPrice').textContent = no ? `${displayCents(no)}${formatSignedChange(predContext.noChange, '¢')}` : '—';
}
...
function exportTracker() {
  const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), records: state.records, completedLadders: state.ladders, ladderAccuracy: summarizeLadderAccuracy(), currentLadder: state.currentLadder }, null, 2)], { type: 'application/json' });
