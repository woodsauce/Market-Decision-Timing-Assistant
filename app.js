...
const APP_VERSION_FILE = 'edge15-entry-minute-9.zip';
...
function exportTracker() {
  const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), records: state.records, completedLadders: state.ladders, ladderAccuracy: summarizeLadderAccuracy(), currentLadder: state.currentLadder, learning: getLearningSummary(), entryMinuteStats: summarizeEntryMinuteTracker() }, null, 2)], { type: 'application/json' });
