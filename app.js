...
const APP_VERSION_FILE = 'edge15-signal-lock-8.zip';
...
function finalizeCurrentLadder(reason = 'closed') {
  ...
  const callSource = ladder.lastCall?.checkpoint ? `${ladder.lastCall.checkpoint}m checkpoint/locked call` : lastCheckpointCall ? `${lastCheckpointCall.minutes}m checkpoint` : recommendation === 'SKIP' ? 'no valid call' : 'live locked call';
...
function renderCompletedLadders() {
  ...
  container.innerHTML = state.ladders.slice(0, COMPLETED_LADDER_LIMIT).map((ladder) => {
    ...
    const source = ladder.callSource ? ` · ${escapeHtml(ladder.callSource)}` : '';
    return `<div class="ladder-record compact-record">
      <div class="ladder-record-head compact-head">
        <div>
          ...
          <span class="small-muted">${new Date(ladder.settledAt || ladder.startedAt).toLocaleTimeString()} · T ${fmtMoney(ladder.targetPrice)} · F ${fmtMoney(ladder.finalPrice)}${source}</span>
        </div>
...
function getTimingSignal(decision = state.decision) {
  ...
  const bestWindow = getLearningSummary().bestEntryWindow || '8m–6m';
...
function getLearningSummary() {
  ...
  const bestEntryWindow = bestEarly && bestEarly[1] > 0 ? `${bestEarly[0]}m` : '8m–6m';
  return {
...
function exportTracker() {
  const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), records: state.records, completedLadders: state.ladders, ladderAccuracy: summarizeLadderAccuracy(), currentLadder: state.currentLadder, learning: getLearningSummary() }, null, 2)], { type: 'application/json' });
